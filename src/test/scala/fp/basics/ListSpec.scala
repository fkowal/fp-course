...
object Lists {
  ...
  def getBrandSet(): Set[Brand] = ???

  // Sorted by brand name.
  def getBrandArray(): Array[Brand] = ???

  def getModels(): Set[Model] = ???

  // Sorted by brand name, then model name.
  def getCars(): List[Car] = ???

  /** See [toMap] */
  def getModelMap(): Map[Model, Car] = ???

  /** See [groupBy] */
  def getCarBrandMap(): Map[Brand, List[Car]] = ???
  ...
  /**
    ...
    */
  def getFordsMModels(): Set[String] = ???
